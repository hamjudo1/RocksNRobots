...
extern int XN297L_payloadIn[2][15];// 15 bytes from the remote.
extern int XN297L_payloadOut[2][15];// 15 bytes from the remote.

// From H8mini_blue_board/Silverware/src/xn297.h

extern void xn_writereg(int reg, int val);
extern int xn_readreg(int reg);
extern void xn_readpayload( int *data , int size );
...
extern void processPacket(int rxdata[15]);
...
#define QUIET_TIME    0xE9 // Let the cortex disable interrupts for a millisecond.
